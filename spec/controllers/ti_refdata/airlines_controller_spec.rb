...
describe TiRefdata::AirlinesController do
  ...
  let!(:airline) do
    ...
    Fabricate(:airline, unified_code: '6X', iata_code: '6X', name: 'Amadeus Six', type_code: '', derived_carrier_int: 6)
  end
  ...
  describe 'GET index with filter' do
    it 'returns our airline when authenticated' do
      ...
      expect(first_json_at '$.airlines[0].derived_carrier_int').to eq(6)
      get :index, code: '7X'
      expect(response.status).to eq(200)
      expect(first_json_at '$.airlines').to have(1).airline
      ...
      expect(first_json_at '$.airlines[0].derived_carrier_int').to eq(7)
    end
    ...
    it 'returns our airlines when authenticated' do
      ...
      expect(first_json_at '$.airlines[1].derived_carrier_int').to eq(7)
    end
  ...
  describe 'GET show' do
    it 'returns our airline when authenticated' do
      ...
      expect(first_json_at '$.airline.derived_carrier_int').to eq(6)
      expect(response.status).to eq(200)
    end
